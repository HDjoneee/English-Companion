...
3. 录音反馈模式：当云端未配置、浏览器转写不可用或网络不稳定时，保留录音并基于音频特征生成即时反馈。

如果没有拿到任何转写文本，应用也不会卡住：系统会自动记录本轮语音回答，保留录音回放，并根据回答时长、有效说话占比、音量、静音和爆音情况给出即时评分，再直接进入下一轮对话。语法和表达纠错会在有转写文本时更精准。
...
- 实时语音：麦克风权限诊断、动态波形、稳定分片转写、录音回放、无转写音频反馈、错误 Toast。
